...
rm -r $HOME/$APOLLO_DIR/modules/map/data/$MAP_DIR
mv -f $MAP_DIR $HOME/$APOLLO_DIR/modules/map/data/
...
docker exec \
    ...
    -e PYTHONPATH="${PYTHONPATH}" \
    -it "${DEV_CONTAINER}" \
    /bin/bash -c "/apollo/scripts/generate_routing_topo_graph.sh -map_dir /apollo/modules/map/data/${MAP_DIR}"

xhost -local:root 1>/dev/null 2>&1

cp -r $HOME/$APOLLO_DIR/modules/map/data/$MAP_DIR ./
