...
class DataBridge(object):
    ...
    def update_npc_vehicle_start(self) -> None:
        for idx in range(len(self.other_actors)):
            ...
            self.other_actors_previous_location.append(npc_location)

    def update_npc_vehicle_motion(self) -> None:
        for idx in range(len(self.other_actors)):
            npc = self.other_actors[idx]
            npc_location = npc.get_location()
            distance = self.other_actors_previous_location[idx].distance(npc_location)
            if distance >= 8:
                self.other_actors_previous_location[idx] = npc_location
            else:
                continue
            npc_wp = self._map.get_waypoint(npc_location)
