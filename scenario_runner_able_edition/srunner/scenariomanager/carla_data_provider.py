...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def set_world(world):
        ...
        #CarlaDataProvider.generate_junction_spawn_points()
        CarlaDataProvider.prepare_map()

        CarlaDataProvider.session = None
        CarlaDataProvider.target_traffic_light = None
        CarlaDataProvider.junction_s1 = {
            "map": "Town06",
            "junction": 332,
            "target_traffic_light_opendrive_id": "1226"
        }
        CarlaDataProvider.actor2roadid_s1_start = {
            "junction": 332,
            "ego_vehicle": 71,
            "npc1": 38,
            "npc2": 38,
        ...
        CarlaDataProvider.actor2roadid_s1_end = {
            "junction": 332,
            "ego_vehicle": (5,4),
            "npc1": (5,3),
            "npc2": (5,4),
        ...
        CarlaDataProvider.junction_s2 = {
            "map": "Town06",
            "junction": 838,
            "target_traffic_light_opendrive_id": "1237"
        }
        CarlaDataProvider.actor2roadid_s2_start = {
            "junction": 838,
            "ego_vehicle": 19,
            "npc1": 34,
            "npc2": 34,
        ...
        CarlaDataProvider.actor2roadid_s2_end = {
            "junction": 838,
            "ego_vehicle": (39,-5),
            "npc1": (39,-4),
            "npc2": (33,4),
        ...
        CarlaDataProvider.junction_s3 = {
            "map": "Town05",
            "junction": 1574,
            "target_traffic_light_opendrive_id": "2405"
        }
        CarlaDataProvider.actor2roadid_s3_start = {
            "junction": 1574,
            "ego_vehicle": 0,
            "npc1": 39,
            "npc2": 16,
        ...
        CarlaDataProvider.actor2roadid_s3_end = {
            "junction": 1574,
            "ego_vehicle": (39,2),
            "npc1": (29,1),
            "npc2": (29,2),
        ...
        CarlaDataProvider.junction_s4 = {
            "map": "Town06",
            "junction": 838,
            "target_traffic_light_opendrive_id": "1237"
        }
        CarlaDataProvider.actor2roadid_s4_start = {
            "junction": 838,
            "ego_vehicle": 19,
            "npc1": 34,
            "npc2": 34,
        ...
        CarlaDataProvider.actor2roadid_s4_end = {
            "junction": 838,
            "ego_vehicle": (39,-5),
            "npc1": (33,4),
            "npc2": (39,-5),
    ...
    def set_traffic_lights(session):
        ...
        for traffic_light in CarlaDataProvider.get_world().get_actors().filter('*traffic_light*'):
            if traffic_light.get_opendrive_id() == osc2trafficlights_dict[session]["target_traffic_light_opendrive_id"]:
                CarlaDataProvider.target_traffic_light = traffic_light
                break
        CarlaDataProvider.target_traffic_light.reset_group()
        target_group_traffic_lights = CarlaDataProvider.target_traffic_light.get_group_traffic_lights()
        for traffic_light in target_group_traffic_lights:
            if traffic_light.get_pole_index() != CarlaDataProvider.target_traffic_light.get_pole_index():
    ...
    def generate_spawn_points():
        ...
        """
        spawn_points = list(CarlaDataProvider.get_map().get_spawn_points())
        CarlaDataProvider._rng.shuffle(spawn_points)
        CarlaDataProvider._spawn_points = spawn_points
        CarlaDataProvider._spawn_index = 0

    @staticmethod
    def road_point_is_junction(transform):
    ...
    @staticmethod
    def generate_road_spawn_points(road_id, junction_id = None, distance = 4.0):
        spawn_points = []
        len_to_junction = 200.0

        waypoints = CarlaDataProvider.get_map().generate_waypoints(distance)
        for wp in waypoints:
            if not wp.is_junction and wp.road_id == road_id:
                if junction_id is None:
                    spawn_points.append(wp.transform)
                else:
                    iter_wp = wp
                    for step in range(int(len_to_junction / distance)):
                        iter_wp = iter_wp.next(distance)[0]
                        if iter_wp.is_junction and iter_wp.junction_id == junction_id:
                            spawn_transform = carla.Transform(carla.Location(wp.transform.location.x,\
                                                                            wp.transform.location.y,\
                                                                            0.3),\
                                                            carla.Rotation(0.0,wp.transform.rotation.yaw,0.0))
                            spawn_points.append(spawn_transform)
                            break
                        elif iter_wp.is_junction and iter_wp.junction_id != junction_id:
                            break

        CarlaDataProvider._rng.shuffle(spawn_points)
        for spawn_point in spawn_points:
            if not CarlaDataProvider.road_point_is_junction(spawn_point):
                return spawn_point

    @staticmethod
    def generate_road_end_points(road_tuple, junction_id = None, distance = 4.0):
        end_points = []
        len_to_junction = 200.0
        road_id = road_tuple[0]
        lane_id = road_tuple[1]

        waypoints = CarlaDataProvider.get_map().generate_waypoints(distance)
        for wp in waypoints:
            if not wp.is_junction and wp.road_id == road_id and wp.lane_id == lane_id:
                if junction_id is None:
                    end_points.append(wp.transform)
                else:
                    iter_wp = wp
                    for step in range(int(len_to_junction / distance)):
                        iter_wp = iter_wp.previous(distance)[0]
                        if iter_wp.is_junction and iter_wp.junction_id == junction_id:
                            end_transform = carla.Transform(carla.Location(wp.transform.location.x,\
                                                                            wp.transform.location.y,\
                                                                            0.3),\
                                                            carla.Rotation(0.0,wp.transform.rotation.yaw,0.0))
                            end_points.append(end_transform)
                            break
                        elif iter_wp.is_junction and iter_wp.junction_id != junction_id:
                            break

        CarlaDataProvider._rng.shuffle(end_points)
        for end_point in end_points:
            if not CarlaDataProvider.road_point_is_junction(end_point):
                return end_point

    @staticmethod
    def generate_junction_spawn_points(distance = 4.0, junction_id = 1574):
        """
        Generate spawn points for the current map
        """
        reversed_road_id_list = [35, 36, 37, 38]
        # All junction id: [53, 207, 359, 396, 562, 720, 838, 979, 1126, 1292, 1427, 1574, 1722, 1882, 1930, 2014, 2062, 2086, 2240, 2296, 2328]
        # 4 way center junction id: [979, 1427, 1574]
        # 3 way junction id: [359, 1882, 1930, 2014, 2062, 2240, 2296, 2328]
        # 3 way junction id with bug: [2014, 2328]
        waypoints = CarlaDataProvider.get_map().generate_waypoints(distance)
        main_junction = None
        juction_road_id_dict = {}
        spawn_points = []
        end_points = []
        ...
        for wp in waypoints:
            if wp.is_junction:
                ...
                junction_wp_list = junction.get_waypoints(carla.LaneType.Driving)
                for wp_tuple in junction_wp_list:
                    juction_road_id_dict[wp_tuple[1].next(distance)[0].road_id] = True
                    juction_road_id_dict[wp_tuple[0].previous(distance)[0].road_id] = True
                junction_dict[junction.id] = len(juction_road_id_dict)
                juction_road_id_dict = {}

        if junction_id is None:
            CarlaDataProvider._rng.shuffle(waypoints)
            for wp in waypoints:
                if wp.is_junction:
                    main_junction = wp.get_junction()
                    break
        else:
            for wp in waypoints:
                if wp.junction_id == junction_id:
                    main_junction = wp.get_junction()
                    break
        
        junction_wp_list = main_junction.get_waypoints(carla.LaneType.Driving)
        for wp_tuple in junction_wp_list:
            juction_road_id_dict[wp_tuple[1].next(distance)[0].road_id] = True
            juction_road_id_dict[wp_tuple[0].previous(distance)[0].road_id] = True

        #import srunner.osc2_stdlib.path as osc2_path #osc2_path.Path.check(spawn_transform)
        len_to_junction = 60.0
        for wp in waypoints:
            if wp.is_junction:
                continue
            if wp.road_id in list(juction_road_id_dict.keys()):
                iter_wp = wp
                for step in range(int(len_to_junction / distance)):
                    if wp.road_id in reversed_road_id_list:
                        iter_wp = iter_wp.next(distance)[0]
                    else:
                        iter_wp = iter_wp.previous(distance)[0]
                    if iter_wp.is_junction and iter_wp.junction_id == main_junction.id:
                        if step < 3:
                            break
                        spawn_transform = carla.Transform(carla.Location(wp.transform.location.x,\
                                                                         wp.transform.location.y,\
                        ...
                        spawn_points.append(spawn_transform)
                        break
                    elif iter_wp.is_junction and iter_wp.junction_id != main_junction.id:
                        break
            if wp.road_id in list(juction_road_id_dict.keys()):
                iter_wp = wp
                for step in range(int(len_to_junction / distance)):
        ...
        CarlaDataProvider._spawn_points = spawn_points
        CarlaDataProvider._spawn_index = 0
        CarlaDataProvider._end_point = CarlaDataProvider._rng.choice(end_points)
        CarlaDataProvider._end_points = end_points
    ...
    def get_waypoint_by_actorname_and_laneid(actor_name: str, lane_id: int):
        ...
        pos = CarlaDataProvider.generate_road_spawn_points(actor2roadid_start[actor_name], actor2roadid_start["junction"])
        wp = CarlaDataProvider.get_map().get_waypoint(pos.location, project_to_road=True, lane_type=carla.LaneType.Driving)

        road_lanes = CarlaDataProvider.get_road_lanes(wp)

        lane = int(float(lane_id))
        if lane > len(road_lanes):
            return None
        else:
            return road_lanes[lane - 1]
