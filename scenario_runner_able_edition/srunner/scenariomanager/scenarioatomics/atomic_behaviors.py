...
class WaitUtilEgoMove(AtomicBehavior):
    ...
    def __init__(self, name="WaitUtilEgoMove"):
        """
        Setup actor
        """
        super(WaitUtilEgoMove, self).__init__(name)
    ...
    def initialise(self):
        ...
        Set start time
        """
        self._start_location = self._ego_vehicle.get_location()
        super(WaitUtilEgoMove, self).initialise()
    ...
    def update(self):
        ...
        if self._start_location.distance(self._ego_vehicle.get_location()) >= 2.0:
            new_status = py_trees.common.Status.SUCCESS
...
class WaypointFollower(AtomicBehavior):
    ...
    def initialise(self):
        ...
        for actor in self._actor_dict:
            ...
            # OSC scenarios
            if len(self._local_planner_dict[actor]._waypoints_queue) == 1 and self.name != "FollowWaypointsABLE":
                ...
                end_transform = CarlaDataProvider.generate_road_end_points(actor2roadid_end[self._actor.attributes["role_name"]], actor2roadid_end["junction"])
                end_location = end_transform.location

                global_planner = GlobalRoutePlanner(carlamap, 2.0)
                route = global_planner.trace_route(start_location, end_location)
                self._local_planner_dict[actor].set_global_plan(route)
                CarlaDataProvider.set_local_planner(self._local_planner_dict[actor])

            if self.name == "FollowWaypointsABLE":
                if self._target_speed == 0.0:
                    persistenttime = 0.8
                else:
                    persistenttime = len(self._local_planner_dict[actor]._waypoints_queue) / self._target_speed / 7.2
                self._draw_waypoints(CarlaDataProvider.get_world(), self._local_planner_dict[actor]._waypoints_queue, persistency=persistenttime)
            else:
                self._draw_waypoints(CarlaDataProvider.get_world(), self._local_planner_dict[actor]._waypoints_queue)
        return True

    def _draw_waypoints(self, world, waypoints, vertical_shift=0.2, persistency=-1):
        """
        ...
        if isinstance(actor, carla.Walker):
            ...
            if self._plan is not None:
                if isinstance(self._plan[0], carla.Location):
                    ...
                    if self.name == "FollowWaypointsABLE":
                        local_planner.set_global_plan(self._plan, clean_queue=False)
                    else:
                        local_planner.set_global_plan(self._plan)
...
class ActorTransformSetter(AtomicBehavior):
    ...
    def __init__(self, actor, transform, physics=True, name="ActorTransformSetter"):
        ...
        self._physics = physics
        self._initialise_loop_counter = 0
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))

    def initialise(self):
        self._initialise_loop_counter += 1
        if self._actor.is_alive and self._initialise_loop_counter <= 1:
            self._actor.set_target_velocity(carla.Vector3D(0, 0, 0))
            self._actor.set_target_angular_velocity(carla.Vector3D(0, 0, 0))
    ...
    def update(self):
        ...
        if self._initialise_loop_counter > 1:
            new_status = py_trees.common.Status.SUCCESS
