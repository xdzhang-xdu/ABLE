...
if __name__ == '__main__':
    mapfolder_inputpath = "map_apollo"
    mapfolder_outputpath = "map"
    mapfolder_list = os.listdir(mapfolder_inputpath)
    for mapfolder in mapfolder_list:
        with open(mapfolder_inputpath + "/" + mapfolder + "/base_map.bin", "rb") as map_bin_file:
            _map = map_pb2.Map()
            _map.ParseFromString(map_bin_file.read())

            map_json = for_map_convert_bin_to_json(_map)
            with open(mapfolder_outputpath + "/" + mapfolder + ".json", 'w') as map_json_file:
                json.dump(map_json, map_json_file)
