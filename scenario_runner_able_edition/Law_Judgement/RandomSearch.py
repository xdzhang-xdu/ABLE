...
if __name__ == "__main__":
    ...
    for session in os.listdir(trace_dir):
        if os.path.isdir(trace_dir + session):
            for trace in os.listdir(trace_dir + session):
                with open(trace_dir + session + "/" + trace) as trace_file:
                    data = json.load(trace_file)

                testcase_list = []
                for idx in range(RS_BATCHES):
                    testcase = TestCaseRandom(data)
                    testcase.testcase_random(TESTCASE_SIZE)
                    testcase_list.extend(testcase.cases[1:])

                for testcase_idx in range(len(testcase_list)):
                    testcase_list[testcase_idx]['ScenarioName'] = "NO_" + str(testcase_idx)

                with open("generated_scenarios/" + session + "_rs.json", 'w') as scenarios_file:
                    json.dump(testcase_list, scenarios_file, indent=4)
