...
class TestCaseRandom:
    def __init__(self, msg):
        ...
        for _i in range(len(testcase_['npcList'])):
            ...
            if 'motion' in testcase_['npcList'][_i]:
                for _j in range(len(testcase_['npcList'][_i]['motion'])):
                    testcase_['npcList'][_i]['motion'][_j].pop('position', None)
        ...
        for _i in range(len(testcase_['pedestrianList'])):
            ...
            if 'motion' in testcase_['pedestrianList'][_i]:
                for _j in range(len(testcase_['pedestrianList'][_i]['motion'])):
                    # testcase_['pedestrianList'][_i]['motion'][_j].pop('position', None)
    ...
    def ego_random(self, lane_config):
        ...
        ###
        _start_lane_max = lane_config[_ego_start_lane]
    ...
    def npc_random(self, original_npc, lane_info, ego_start):
        ...
        ###
        _npc_start_lane = correct_lane_key(lane_config, _npc['start']['lane_position'])
        ###
        _start_lane_length = lane_config[_npc_start_lane]
        ...
        if 'motion' in _npc:
            for _i in range(len(_npc['motion'])):
                ...
                ###
                wp_i_lane = correct_lane_key(lane_config, wp_i['lane_position'])
                ###
        ...
        if _npc['destination'] is not None:
            ...
            ###
            _npc_destination_lane = correct_lane_key(lane_config, _npc['destination']['lane_position'])
            ###
            if _npc_destination_lane == _current_lane:
                if 'motion' in _npc:
                    _npc_destination_min = _npc['motion'][-1]['lane_position']['offset']
                else:
                    _npc_destination_min = _npc['start']['lane_position']['offset']
                _npc_destination_max = _current_length
            else:
                _npc_destination_min = offset_offset
                _npc_destination_max = lane_config[_npc_destination_lane]
    ...
    def testcase_random(self, num):
        for _i in range(num):
            ...
            _new_case['ScenarioName'] = "No_{}".format(_i + 1)
            if True:
...
if __name__ == "__main__":
    initial_path = "trace/"
    randomized_path = "traceset_randomized/"

    for initial_subpath in os.listdir(initial_path):
        if os.path.isdir(initial_path + initial_subpath):
            for trace in os.listdir(initial_path + initial_subpath):
                print(trace)
                if not os.path.isdir(randomized_path + initial_subpath):
                    os.mkdir(randomized_path + initial_subpath)
                with open(initial_path + initial_subpath + "/" + trace) as trace_file:
                    data = json.load(trace_file)
                for idx in range(8):
                    if not "temp" in trace and not os.path.isfile(randomized_path + initial_subpath + "/" + trace.replace("trace", "traceset_" + str(idx))):
                        testcase = TestCaseRandom(data)
                        testcase.testcase_random(20)
                        with open(randomized_path + initial_subpath + "/" + trace.replace("trace", "traceset_" + str(idx)), "w") as traceset_file:
                            json.dump(testcase.cases, traceset_file, indent=4)
