...
class ScenarioRunner(object):
            ...
            while ego_vehicle_missing:
                ...
                for ego_vehicle in ego_vehicles:
                    ...
                    if not ego_vehicle_found:
                        ...
                        break
                nowTime = time.time()
                if nowTime - preTime > 5.0:
                    preTime = nowTime
                    print("Ego vehicles still missing")
    ...
    def _load_and_run_scenario(self, config):
        ...
        try:
            ...
            #OSC2
                ...
                _, osc_file = os.path.split(config.name)
                session, ext = os.path.splitext(osc_file)
            ...
            #ABLE
            elif self._args.able:
                _, able_file = os.path.split(self._args.able)
                session, ext = os.path.splitext(able_file)
                CarlaDataProvider.set_traffic_lights(session.replace("trace_", "").replace("temp_", ""))
        ...
        try:
            ...
            self.world.manager = self.manager
            
            if self._args.waitForEgo and not self._args.able is None:
                with open(self._args.able, "r") as able_file:
                    self.manager.able_data = json.load(able_file)
            
            self.manager.run_scenario()
            scenario_path = "trace/"
            scenario_filename = None
            # OSC2 scenario
            if not self._args.osc2 is None:
                _, scenario_filename = os.path.split(self._args.osc2)
            # ABLE replay scenario
            if not self._args.able is None and os.path.isfile(self._args.able):
                _, scenario_filename = os.path.split(self._args.able)
            # Others
    ...
    def _run_able(self):
        """
        Run a scenario based on able trace file
        """

        # Load the scenario configurations provided in the config file
        if type(self._args.able) == dict:
            print("Using scenario data.")
        elif os.path.isfile(self._args.able):
            print("Using file:", self._args.able)
        else:
            print("File does not exist")
            self._cleanup()
            return False

        config = ABLEScenarioConfiguration(self._args.able, self.client)
...
def main():
    ...
    parser.add_argument('--able', help='Provide an able definition')
    parser.add_argument(
    ...
    if not arguments.scenario and not arguments.openscenario and not arguments.route and not arguments.osc2 and\
    not arguments.able:
