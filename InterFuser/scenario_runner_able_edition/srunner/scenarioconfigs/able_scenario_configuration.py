...
class ABLEScenarioConfiguration(ScenarioConfiguration):
    ...
    def parse_json_configuration(self):
        ...
        """
        self.weather.precipitation = self.data["weather"]["rain"]
        self.weather.cloudiness = self.data["weather"]["sunny"]
        self.weather.wetness = self.data["weather"]["wetness"]
        self.weather.fog_density = self.data["weather"]["fog"]
    ...
    def set_ego_vehicle(self):
        start_lane_position = self.data["ego"]["start"]["lane_position"]
        spawn_point = CarlaDataProvider.get_map().get_waypoint_xodr(
            int(start_lane_position['lane'].replace("lane_", "")),
            start_lane_position['roadID'],
            start_lane_position['offset']
        ).transform

        self.trajectory.append(carla.Location(x=spawn_point.location.x,
                                              y=spawn_point.location.y,
                                              z=spawn_point.location.z))

        end_lane_position = self.data["ego"]["destination"]["lane_position"]
        end_point = CarlaDataProvider.get_map().get_waypoint_xodr(
            int(end_lane_position['lane'].replace("lane_", "")),
            end_lane_position['roadID'],
            end_lane_position['offset']
        ).transform

        self.trajectory.append(carla.Location(x=end_point.location.x,
